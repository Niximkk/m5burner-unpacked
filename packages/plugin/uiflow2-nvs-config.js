...
const fillCSV = ({server, ssid, pwd, sntp0, sntp1, sntp2, timezone, bootOpt, powerMode}) => {
...
boot_option,data,u8,${bootOpt}`
  ...
  if(powerMode !== undefined && powerMode !== '') {
    let powerModeExt = `power_mode,data,string,${powerMode}`
    csv += `\n${powerModeExt}`
  }
