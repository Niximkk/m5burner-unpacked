...
function getMac(port) {
  return new Promise(resolve => {
    ...
    process.on('close', code => {
      if(code === 0) {
        let mac = ''
        if(output.indexOf('ESP32-C6') >= 0){
          // 芯片为ESP32-C6时MAC地址应为BASE MAC地址
          mac = output.substring(output.indexOf('BASE MAC: ') + 10, output.indexOf('BASE MAC: ') + output.substring(output.indexOf('BASE MAC: ')).indexOf('\n'))
        } else {
          mac = output.substring(output.indexOf('MAC: ') + 5, output.indexOf('MAC: ') + output.substring(output.indexOf('MAC: ')).indexOf('\n'))
        }
